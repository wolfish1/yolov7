...
class conv_bn_hswish(nn.Module):
    ...
    def __init__(self, c1, c2, k, stride):
        super(conv_bn_hswish, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, stride, 1, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = h_swish()
...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        y_w = y_w.permute(0,1,3,2)          # [B,C,W,1] -> [B,C,1,W]
...
class ECAConv(nn.Module):
    ...
    YAML 寫法：
        [-1, 1, ECAConv, [256, 1, 1]]
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, k_size=3):
        super().__init__()
        self.conv = Conv(c1, c2, k, s, p, g, act)
        self.att  = ECA(c2, k_size=k_size)
